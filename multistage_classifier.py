...
from predict import (
    get_device,
    get_meta_dict_from_load_dir,
    get_subset_dataset_and_loader,
    load_model,
    prepare_results_csv,
...
def main(data_dir, subset, model_paths, out_dir):
    ...
    results_file = run_twostep_classifier(
        dataset, subset, dataloader, binary_model, trinary_model
    )
...
def run_twostep_classifier(dataset, subset, dataloader, binary_model, trinary_model):
    # get where we will write results to
    class_names = ["diestrus", "not_diestrus", "proestrus", "estrus", "metestrus"]
    ...
    with tqdm(desc="Twostep", total=len(dataset)) as pbar:
        for input_tensor, label, (path,) in dataloader:
            ...
            with torch.set_grad_enabled(False):
                binary_output = torch.nn.Softmax(dim=1)(binary_model(input_tensor))
            ...
            if prediction_1 == 0:
                ...
                trinary_output = torch.zeros(1, 3, device=device)  # filler output
            ...
            # step 2, only if needed
                ...
                raise ValueError("Some shit happened, first prediction was not binary.")
