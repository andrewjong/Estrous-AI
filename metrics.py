...
CONFUSION_MATRIX_IMG_NAME = "confusion_matrix.png"
METRICS_TEXT_NAME = "performance_metrics.json"
...
def create_all_metrics(predictions_files, outdir, name_prefix=""):
    ...
    for f in predictions_files:
        y_true, y_pred, classes = get_prediction_df_and_classes(f)
        # create other metrics
        cm_outfile = os.path.join(outdir, name_prefix + CONFUSION_MATRIX_IMG_NAME)
        all_metrics = {
            "accuracy": skm.accuracy_score(y_true, y_pred, normalize=True),
            "confusion_matrix": str(
                write_confusion_matrices(y_true, y_pred, classes, cm_outfile)
            ),
        }

        weighted_scores = {
            score
            + average: getattr(skm, score + "_score")(y_true, y_pred, average=average)
            for score in ("precision", "recall", "f1")
            for average in ("micro", "macro", "weighted")
        }

        all_metrics.update(weighted_scores)

        out_file = os.path.join(outdir, name_prefix + METRICS_TEXT_NAME)
        with open(out_file, "w") as out:
            json.dump(all_metrics, out, indent=4)


def get_prediction_df_and_classes(predictions_file):
    df = pd.read_csv(predictions_file)
    try:
        df = df.drop(columns=["not_diestrus"])  # for two-stage classifier
    except KeyError:
        pass
    ...
    return df["label"], df["predicted"], classes


def write_confusion_matrices(y_true, y_pred, classes, outfile):
    conf_matrix = skm.confusion_matrix(y_true, y_pred, labels=classes)
    create_confusion_matrix_plots(conf_matrix, classes, outfile)
    return conf_matrix


def create_confusion_matrix_plots(confusion_matrix, classes, out_file):
    ...
    fig = plot_confusion_matrix(confusion_matrix, classes)
    fig.savefig(out_file)
    # plot normalized confusion matrix
    fig_normalized = plot_confusion_matrix(confusion_matrix, classes, normalize=True)
